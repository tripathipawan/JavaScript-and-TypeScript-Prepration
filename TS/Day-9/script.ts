...
let students: string[] = ["Pawan", "Rahul", "Amit", "Priya", "Neha"];

let firstName: string  = students[0];                    // "Pawan"
let lastName: string   = students[students.length - 1];  // "Neha"
let total: number      = students.length;                // 5
let isArray: boolean   = Array.isArray(students);        // true
let thirdStudent: string = students[2];                  // "Amit"
...
let nums: number[] = [10, 20, 30]; // ✅ number[] not number
...
function getArrayStats(scores: number[]): { highest: number; lowest: number; average: number } {
  ...
  let average: number = parseFloat((sum / scores.length).toFixed(2));
